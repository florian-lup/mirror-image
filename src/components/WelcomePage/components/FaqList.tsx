  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2 max-w-xl mx-auto px-3 sm:px-0">
      {questions.map((question, index) => (
        <button
          key={index}
          className="text-left p-2.5 sm:p-3 rounded-xl transition-all duration-300 backdrop-blur-sm
        ...
        >
          ...
          <span className="text-[12px] sm:text-[13px] leading-relaxed text-neutral-300 group-hover:text-neutral-100 relative z-10">
            {question.text}
          </span>
        </button>
      ))}
